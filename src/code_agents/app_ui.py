...
class AgentController:
    ...
    def __init__(self, agent_dict: dict[str, type[CodeAgent]], agent_names: list[str]) -> None:
    ...
    def get_changed_files(self, workspace: Path) -> list[Path]:
    ...
    def sync_to_home(self, workspace: Path, repo_url: str) -> tuple[int, Path]:
        """
        ...
        changed_files = self.get_changed_files(workspace)
        copied = 0
    ...
    def commit_to_home(self, workspace: Path, repo_url: str, message: str) -> tuple[bool, str]:
        """Sync changed files to the user's home repo and commit them."""
        copied, target_root = self.sync_to_home(workspace=workspace, repo_url=repo_url)
        if copied == 0:
    ...
    @st.cache_resource
    def get_agent(_self, agent_type: str, repo_url: str, branch: str) -> CodeAgent[Any]:
        """Instantiate and cache agent instance."""
        return _self.agent_dict[agent_type](repo_url, branch)

    def initialize_agent(self, agent_type: str, repo_url: str, branch: str) -> CodeAgent[Any]:
        """Initialize and store agent in session state."""
        selected_agent: CodeAgent[Any] = self.get_agent(agent_type=agent_type, repo_url=repo_url, branch=branch)
        return selected_agent

    def execute_agent_task(self, agent: CodeAgent[Any], task: str, system_prompt: str, command: AgentCommand) -> None:
        """Execute agent with given task and system prompt."""
        sys_prompt = AVAILABLE_PROMPTS[system_prompt]

        if sys_prompt != SYS_EMPTY:
            task = "<SYSTEM PROMPT>\n" + sys_prompt + "\n</SYSTEM PROMPT>" + "\n\n" + "<USER PROMPT>\n" + task + "\n</USER PROMPT>"

        agent.run(task=task, command=command)

    def run_workspace_test(self, agent: CodeAgent[Any]) -> None:
...
def agent_controls(controller: AgentController) -> None:
    ...
    if "selected_agent" not in st.session_state:
        ...
        selected_agent_name: str = st.selectbox("Select Agent", options=controller.agent_names, key="agent_selector")
        ...
        if repo_url:
            if "branches" not in st.session_state or st.session_state.cached_repo_url != repo_url:
                with st.spinner("Fetching branches..."):
                    st.session_state.branches = controller.get_remote_branches(repo_url)
        ...
        with st.expander("", expanded=False):
            ...
            with col_1:
                ...
                st.button(
                    ...
                    use_container_width=True,
                    type="secondary",
                    on_click=lambda: controller.run_workspace_test(selected_agent),
                )
            ...
            with col_2:
                ...
                if st.button("Sync Code", use_container_width=True):
                    count, target_root = controller.sync_to_home(
                ...
                if st.button("Commit Changes", key="commit_changes_button"):
                    if not commit_msg.strip():
                        ...
                        with st.spinner("Syncing and committing changes to home repository..."):
                            success, msg = controller.commit_to_home(
                                workspace=selected_agent.path_agent_workspace,
                                repo_url=selected_agent.repo_url,
